  ...
  return <Layout>
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="glass-card rounded-lg p-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 ">
            <div className="relative">
              ...
              <Input placeholder="Search tasks..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 border-[#eae8e1]" />
            </div>
        ...
        <div className="space-y-6">
          {/* Active Tasks */}
          {activeTasks.length > 0 && <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {activeTasks.map(task => <div key={task.id} className="relative group">
                    <TaskCard task={task} onToggle={toggleTask} onEdit={openEditModal} />
                    <Button variant="ghost" size="sm" className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity bg-background/80 hover:bg-destructive hover:text-destructive-foreground" onClick={() => deleteTask(task.id)}>
          ...
          {/* Completed Tasks */}
          {completedTasks.length > 0 && <div className="space-y-4">
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-semibold text-foreground">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {completedTasks.map(task => <div key={task.id} className="relative group">
                    <TaskCard task={task} onToggle={toggleTask} onEdit={openEditModal} />
                    <Button variant="ghost" size="sm" className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity bg-background/80 hover:bg-destructive hover:text-destructive-foreground" onClick={() => deleteTask(task.id)}>
                      <Trash2 className="w-4 h-4" />
                    </Button>
          ...
          {/* Empty State */}
          {filteredTasks.length === 0 && <div className="glass-card rounded-lg p-12 text-center">
      ...
      <TaskModal isOpen={isTaskModalOpen} onClose={closeTaskModal} onSave={editingTask ? handleEditTask : handleCreateTask} task={editingTask} />
    </Layout>;
};
